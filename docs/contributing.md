...
- **Architecture decisions** — decisions that are hard to reverse, contested, or
  ...
  `npx @adrkit/cli@0.13.0 new "<title>"`, fill in `affects` so the decision is
  locatable by path, and check what already governs a file with
  `npx @adrkit/cli@0.13.0 explain <path>`. Do **not** write an ADR for naming,
  ...
  (`npm i -g @adrkit/cli@0.13.0`, which provides `adr`) or point
