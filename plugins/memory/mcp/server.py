...
Standard library only. Every operation shells out to `memory-provider.py` with
exact argv and no shell, so contract validation, project isolation, review
state, and reconciliation have exactly one implementation and the CLI and MCP
...
PROVIDER = Path(__file__).resolve().parents[1] / "scripts" / "memory-provider.py"
